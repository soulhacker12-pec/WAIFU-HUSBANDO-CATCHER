...
async def harem(update: Update, context: CallbackContext, page=0) -> None:
    ...
    # Get the user's selected mode from Redis
    mode = r.get(f'hmode:{user_id}')
    if mode:
    ...
    # Filter characters based on mode if set to Celestial
    if mode == 'Celestial':
        unique_characters = [char for char in unique_characters if char.get('rarity') == 'Celestial']
...
async def hmode(update: Update, context: CallbackContext, mode: str) -> None:
    user_id = update.effective_user.id

    # Store the selected mode in Redis
    r.set(f'hmode:{user_id}', mode)
    
    await harem(update, context)  # Call the harem function to update the display based on the new mode
